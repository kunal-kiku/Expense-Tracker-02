...
exports.createTransaction = async (req, res) => {
    try {
        const { title, amount, type, category, note, tags, date } = req.body;
...
exports.getTransactionSummary = async (req, res) => {
    try {
        ...
        transactions.forEach(txn => {
            // Use type field if available, fall back to sign-based for old data
            if (txn.type === 'income') {
                income += txn.amount;
            } else if (txn.type === 'expense') {
                expense += txn.amount;
            } else {
                // Backward compatibility for old transactions without type
