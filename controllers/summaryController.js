...
const getBudgetSummary = async (req, res) => {
    try {
        ...
        // 2️⃣ Total Income & Expenses
        // First add an effectiveType field that falls back to sign-based for old data
        const txAgg = await Transaction.aggregate([
            { $match: { userId } },
            {
                $addFields: {
                    effectiveType: {
                        $ifNull: [
                            "$type",
                            { $cond: [{ $gte: ["$amount", 0] }, "income", "expense"] }
                        ]
                    }
                }
            },
            {
                $group: {
                    _id: null,
                    totalIncome: {
                        $sum: { $cond: [{ $eq: ["$effectiveType", "income"] }, { $abs: "$amount" }, 0] }
                    },
                    totalExpenses: {
                        $sum: { $cond: [{ $eq: ["$effectiveType", "expense"] }, { $abs: "$amount" }, 0] }
                    },
                    totalCount: { $sum: 1 }
                }
            }
        ...
        // 4️⃣ Savings = Income - Expenses (can be negative)
